<script lang="ts">
	...
	const cleanup = registerComponent('routes/admin/queues/+page', {
		filePath: 'src/routes/admin/queues/+page.svelte'
	});
	...
	function handleQueueClick(queueName: string) {
		goto(`/admin/queues/${encodeURIComponent(queueName)}`);
	}
	...
	onMount(() => {
		...
		if (import.meta.env.DEV) {
			const clearViewId = setViewId('page:/admin/queues');
			return () => {
				if (pollingInterval) {
