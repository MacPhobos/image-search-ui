<script lang="ts">
	...
	const cleanup = registerComponent('routes/admin/queues/[queueName]/+page', {
		filePath: 'src/routes/admin/queues/[queueName]/+page.svelte'
	});

	// DEV: Set view ID for DevOverlay breadcrumb and component cleanup
	onMount(() => {
		if (import.meta.env.DEV) {
			const clearViewId = setViewId('page:/admin/queues/[queueName]');
			return () => {
				cleanup();
				clearViewId?.();
			};
