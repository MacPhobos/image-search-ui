<script lang="ts">
	...
	const cleanup = registerComponent('routes/admin/vectors/+page', {
		filePath: 'src/routes/admin/vectors/+page.svelte'
	});
	...
	onMount(() => {
		...
		if (import.meta.env.DEV) {
			const clearViewId = setViewId('page:/admin/vectors');
			return () => {
				cleanup();
				clearViewId?.();
			};
